...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="独立版 LLM-GCL 4:4:1:1 模型")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train = subparsers.add_parser("train", help="执行 4:4:1:1 训练、贝叶斯优化与阶段门控")
    ...
    train.add_argument("--strict-signature", action="store_true", help="要求成员、规则和连续参数逐值复现冻结的 4411 模型，否则中止")
...
def main() -> None:
    ...
    output = pd.DataFrame({"y_prob": probability, "y_pred": model.predict(frame), "threshold": model.threshold})
