...
def _mean(pool: dict[str, dict[str, Any]], member_ids: list[str], split: str) -> np.ndarray:
...
def select_xgb_lr(registry: pd.DataFrame, pool: dict[str, dict[str, Any]]) -> dict[str, Any]:
    y_val = next(iter(pool.values()))["val_true"]
    ...
    def best_for(member_ids: list[str]) -> dict[str, Any]:
        ...
        for weight in weights:
            metrics = best_scanned_metrics(y_val, _blend_xgb_lr(pool, member_ids, "val", weight))
    ...
    return {
        ...
        "val_metrics": {name: float(current[name]) for name in AUC_RANKING},
    }
...
def select_cat(registry: pd.DataFrame, pool: dict[str, dict[str, Any]], base: dict[str, Any]) -> dict[str, Any]:
    ...
    y_val = next(iter(pool.values()))["val_true"]
    baseline = best_scanned_metrics(y_val, _blend_xgb_lr(pool, xgb_ids + lr_ids, "val", float(base["xgb_weight"])))
            ...
            for wx in values:
                for wl in values:
                    ...
                    if wc < 0.05 or wc > 0.25:
                        continue
                    probability = wx * _mean(pool, xgb_ids, "val") + wl * _mean(pool, lr_ids, "val") + wc * _mean(pool, list(subset), "val")
                    metrics = best_scanned_metrics(y_val, probability)
    ...
    if not candidates:
        return {"base_best_member_ids": xgb_ids + lr_ids, "selected_cat_member_ids": [], "xgb_weight": base["xgb_weight"], "lr_weight": base["lr_weight"], "cat_weight": 0.0, "threshold": base["threshold"]}
    selected = sorted(candidates, key=rank_key, reverse=True)[0]
    return {"base_best_member_ids": xgb_ids + lr_ids, **selected}
...
def raw_upstream_frames(pool: dict[str, dict[str, Any]], selection: dict[str, Any], splits: list[str]) -> dict[str, pd.DataFrame]:
    ...
    for split in splits:
        ...
        xgb_mean = _mean(pool, xgb_ids, lookup)
        lr_mean = _mean(pool, lr_ids, lookup)
        cat_mean = _mean(pool, cat_ids, lookup)
        frames[split] = pd.DataFrame({"dataset": split, "y_true": pool[base_ids[0]][f"{lookup}_true"], "xgb_mean_prob": xgb_mean, "lr_mean_prob": lr_mean, "cat_mean_prob": cat_mean})
    return frames
